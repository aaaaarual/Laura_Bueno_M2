    ...
    const DBPATH = '../data/ponderada.db';
    ...
    app.post('/insereUsuario', urlencodedParser, (req, res) => {
        ...
        res.setHeader('Access-Control-Allow-Origin', '*'); 
        var db = new sqlite3.Database(DBPATH); // Abre o banco
        sql = "INSERT INTO pessoa (nome, email, telefone, endereco) VALUES ('" + req.body.nome + "', '" + req.body.email + "', '" + req.body.telefone + "', '" + req.body.endereco +"')";
        console.log(sql);
        db.run(sql, [],  err => {
            if (err) {
                throw err;
    ...
    app.post('/atualizaUsuario', urlencodedParser, (req, res) => {
        res.statusCode = 200;
        res.setHeader('Access-Control-Allow-Origin', '*'); 
        sql = "UPDATE pessoa SET nome='" + req.body.nome + "', email = '" + req.body.email + "' , telefone='" + req.body.telefone + " ', endereco= '" + req.body.endereco + "' WHERE id='" + req.body.id + "'";
        console.log(sql);
        var db = new sqlite3.Database(DBPATH); // Abre o banco
        db.run(sql, [],  err => {
            if (err) {
    ...
    app.get('/habilidade', (req, res) => {
        ...
        res.setHeader('Access-Control-Allow-Origin', '*');
        var db = new sqlite3.Database(DBPATH); // Abre o banco
        var sql = 'SELECT * FROM habilidade ORDER BY HABILIDADE_ID';
            db.all(sql, [],  (err, rows ) => {
                if (err) {
                    throw err;
                }
